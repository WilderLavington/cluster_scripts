...
def train(params):
    ...
    if params.restore.G and not params.restore.reset:
        G.load_state_dict(torch.load(params.restore.G))

    if params.restore.D and not params.restore.reset:
    ...
    for epoch in range(params.train.epochs):
        ...
        if epoch % 100 == 0:
            gen_img = 255 * (d_fake_input.cpu().data.numpy() / 2.0 + 0.5)
            print(gen_img.shape)
            fig = plt.gcf()
            plt.imshow(gen_img[0].transpose((1, 2, 0)).astype(int))
            fig.canvas.set_window_title('Random cat')
            plt.savefig('cat_pics/example'+str(epoch)+'.png')

        print('Epoch: {0}, D: {1}, G: {2}'.format(
            epoch, d_fake_error.data, g_error.data))
